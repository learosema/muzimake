...
path_t *path_join(path_t *path, const char *path_string)
{
	...
	size_t newlen = path1_end + 3 + strlen(path_string) - path2_start;
	char *newbuf = (char *)realloc(path->str, newlen);
	if (newbuf == NULL) {
		return NULL; /* realloc fehlgeschlagen, path bleibt unverändert nutzbar */
	}
	/* newbuf enthält bereits den alten Inhalt bis path1_end+1 (durch realloc) */
	...
	path->str = newbuf;
	path->len = newlen - 1;   /* newlen enthält den Platz für '\0', len selbst nicht */	return path;
}
